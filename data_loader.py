...
num_test_samples = [1000, 50]
# num_test_samples = [100, 40]
...
def make_longtailed_imb(max_num, class_num, gamma):
    ...
    print(mu)
    class_num_list = [int(max_num * np.power(mu, i)) for i in range(class_num)]
    return list(class_num_list)


def get_val_test_data(
    dataset, num_sample_per_class, shuffle=False, random_seed=0
):
    """
    ...
    for i in range(length):
        ...
        if num_sample_per_class[label] > (9 * tgt[label] / 10):
...
x, y = torch.load("ds.pt")
ds = TensorDataset(x, y)
train_ds = torch.utils.data.Subset(ds, list(range(len(ds) - 10000)))
test_ds = torch.utils.data.Subset(ds, list(range(len(ds) - 10000, len(ds))))
# train_ds = torch.utils.data.Subset(ds, list(range(len(ds) - 50)))
# test_ds = torch.utils.data.Subset(ds, list(range(len(ds) - 50, len(ds))))


def get_imbalanced(dataset, num_sample_per_class, batch_size):
    print("Building CV {} data loader with {} workers".format(dataset, 8))
    ds = []
    ...
    # train_ds = dataset_(root=DATA_ROOT, train=True, download=False)
    train_in_idx = get_imbalanced_data(train_ds, num_sample_per_class)
    ...
    # test_ds = dataset_(root=DATA_ROOT, train=False, download=False)
    val_idx, test_idx = get_val_test_data(test_ds, num_test_samples)
