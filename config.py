...
def parse_args():
    ...
    parser.add_argument(
        "--dataset",
        required=True,
        choices=["cifar10", "cifar100", "blah"],
        help="Dataset",
    )
    parser.add_argument(
        "--decay", default=2e-4, type=float, help="weight decay"
    )
    parser.add_argument(
        "--no-augment",
        dest="augment",
    ...
    parser.add_argument(
        ...
        default=160,
        type=int,
        help="Deferred strategy for re-balancing",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--smote", "-s", action="store_true", help="oversampling"
    )
    parser.add_argument(
        "--cost", "-c", action="store_true", help="oversampling"
    )
    parser.add_argument(
        "--effect_over",
        action="store_true",
...
ARGS = parse_args()
SEED = ARGS.seed if ARGS.seed is not None else np.random.randint(10000)
np.random.seed(SEED)
...
N_SAMPLES_PER_CLASS_BASE = [30000, 1500]
# N_SAMPLES_PER_CLASS_BASE = [250, 50]
...
train_loader, val_loader, test_loader = get_imbalanced(
    DATASET, N_SAMPLES_PER_CLASS_BASE, BATCH_SIZE
)
# print(torch.unique(train_loader))
...
def evaluate(net, dataloader, logger=None):
    ...
    msg = (
        "Loss: %.3f | Acc: %.3f%% (%d/%d) | Major_ACC: %.3f%% | Neutral_ACC: %.3f%% | Minor ACC: %.3f%% %d %d"
        % (
            results["loss"],
            results["acc"],
            correct,
